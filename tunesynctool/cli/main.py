...
@click.option('--spotify-redirect-uri', 'spotify_redirect_uri', help='Spotify redirect URI.')
...
@click.option(
    '--subsonic-legacy-auth/--no-subsonic-legacy-auth',
    'subsonic_legacy_auth',
    default=None,
    help='Whether to enable legacy authentication for the Subsonic server.',
)
...
def cli(
    ...
    subsonic_base_url: str | None,
    subsonic_port: int | None,
    subsonic_username: str | None,
    subsonic_password: str | None,
    subsonic_legacy_auth: bool | None,
    deezer_arl: str | None,
    youtube_request_headers: str | None,
):
    ...
    ctx.obj['config'] = replace(Configuration.from_env(), **overrides)
